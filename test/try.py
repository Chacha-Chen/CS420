...
label = np.fromfile("./data/label_train",dtype=np.uint8)
#reshape the matrix
#data=getdata.X_train
data_test = getdata.X_test
...
##	im.save(str(i) + ".png")
#	dst=morphology.remove_small_objects(data[i],min_size=4,connectivity=1)
#	im_changed = Image.fromarray(dst)
##	im_changed.save(str(i) + "_new.png")
...
for i in range(10):
    for j in range(10):
        index=i*10+j
        axes[i][j].imshow(data[index], cmap='gray_r', interpolation='none')
        axes[i][j].set_xticks([])
        axes[i][j].set_yticks([])
#
fig, axes = plt.subplots(10, 10, figsize=(10, 10))
...
for i in range(10):
    for j in range(10):
        index=i*10+j
        axes[i][j].imshow(nd_data[index], cmap='gray_r', interpolation='none')
        axes[i][j].set_xticks([])
        axes[i][j].set_yticks([])
...
for i in range(0,200):
    nd_testdata_deskew[i]=deskew(nd_testdata[i])
    
fig, axes = plt.subplots(10, 10, figsize=(10, 10))
for i in range(10):
    for j in range(10):
        index=i*10+j
        axes[i][j].imshow(nd_testdata_deskew[index], interpolation='none')
        axes[i][j].set_xticks([])
        axes[i][j].set_yticks([])
...
#pkl_file = open('./data/data_test_reducedn.pkl', 'rb')
#
...
#    for i in range(45):
#        for j in range(45):
#            if (img_cubao[k][i,j]<=0.5*255):
#                img_cubao[k][i,j]=0
#            else:
#                img_cubao[k][i,j]=1
                
#for i in range(100):
#    img_cubao[i]=morphology.remove_small_objects(img_cubao[i],
#             min_size=4,connectivity=10)
